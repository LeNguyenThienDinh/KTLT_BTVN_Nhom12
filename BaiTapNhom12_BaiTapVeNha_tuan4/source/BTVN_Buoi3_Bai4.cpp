...
void sapXepDongLeTangChanGiam(int **maTran, int n)
{
	for (int i = 0; i < n; ++i)
	{
		for (int j = 0; j < n - 1; ++j)
		{
			for (int k = j + 1; k < n; ++k)
			{
				if ((i % 2 == 0 && maTran[i][j] < maTran[i][k]) || (i % 2 != 0 && maTran[i][j] > maTran[i][k]))
				{
					int temp = maTran[i][j];
...
void sapXepCotLeGiamChanTang(int **maTran, int n)
{
	for (int j = 0; j < n; ++j)
	{
		for (int i = 0; i < n - 1; ++i)
		{
			for (int k = i + 1; k < n; ++k)
			{
				if ((j % 2 == 0 && maTran[i][j] > maTran[k][j]) || (j % 2 != 0 && maTran[i][j] < maTran[k][j]))
				{
					int temp = maTran[i][j];
...
void diChuyenChanLenTrenLeXuongDuoi(int **maTran, int n)
{
	int *arrChan = (int *)malloc(n * n * sizeof(int));
	int *arrLe = (int *)malloc(n * n * sizeof(int));
	...
	for (int i = 0; i < n; ++i)
	{
		...
		{
			if (maTran[i][j] % 2 == 0)
				arrChan[chanCount++] = maTran[i][j];
			else
				arrLe[leCount++] = maTran[i][j];
		}
	}
	...
	for (int i = 0; i < n; ++i)
	{
		...
		{
			if (k < chanCount)
				maTran[i][j] = arrChan[k++];
			else
				maTran[i][j] = arrLe[k++ - chanCount];
		}
	}
...
int main()
{
	...
	while (1)
	{
		...
		printf("8, sap xep dong le tang dong chan giam\n");
		...
		case 7:
			...
			break;
		case 8:
			sapXepDongLeTangChanGiam(maTran, n);
			printf("ma tran sau khi sap xep dong le tang chan giam la:\n");
			inMaTranVuong(maTran, n);
			break;
		case 9:
